...
const Media = ({ heading }) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response.data) {
          ...
          if(idFromUrl) {
            const videoFromUrl = data.find(video => video.id == parseInt(idFromUrl));
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-6 py-16">
      ...
      {list.fetching ? <div className="flex items-center justify-center"><Loader/></div> : list.success ?  <div className="flex flex-col lg:flex-row gap-8">
        ...
        {/* <div className="w-full lg:w-[380px] flex-shrink-0 lg:sticky lg:top-[140px] lg:self-start">
          <div className={`text-xs font-black uppercase tracking-widest mb-6 ${isDayMode ? 'text-slate-500' : 'text-slate-400'}`}>{t.mediaPage.allVideos}</div>
          ...
          <div className="space-y-4">
            {categories.map(category => {
              ...
              return (
                <div key={category} className={`border-2 ${isDayMode ? 'border-slate-200' : 'border-slate-700'} overflow-hidden transition-all`}>
                  <button
                    onClick={() => toggleCategory(category)}
                    className={`w-full flex items-center justify-between p-4 ${isExpanded
                  ...
                  >
                    <div className="flex flex-col items-start gap-2">
                      <div className="flex items-center gap-2">
                        <Tag size={14} className={isExpanded ? 'text-white' : 'text-orange-600'} />
                        <h3 className="text-[11px] font-black uppercase tracking-wider">{category}</h3>
                      </div>
                      <span className={`text-[9px] font-bold ${isExpanded ? 'text-orange-200' : isDayMode ? 'text-slate-500' : 'text-slate-400'}`}>
                  ...
                  {isExpanded && (
                    <div className={`${isDayMode ? 'bg-slate-50' : 'bg-slate-800/50'}`}>
                          ...
                          >
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <h4 className={`text-[11px] font-black uppercase tracking-tight mb-1 line-clamp-2 ${isDayMode ? 'text-black group-hover:text-orange-600' : 'text-white group-hover:text-orange-600'} transition-colors`}>
                                  {video.title}
                                </h4>
                                <p className={`text-[9px] ${isDayMode ? 'text-slate-500' : 'text-slate-400'}`}>{video.date}</p>
                              </div>
                            </div>
                          </div>
                        ))}
